...
with app.app_context():
    ...
    for y in x:
        meal = Meal(name=y, category=meal_cat, group=user.group, client_id=str(uuid.uuid4()))
        db.session.add(meal)
        db.session.commit()
    ...
    x = ast.literal_eval(line)
    for y in x:
        meal = Meal(name=y, category=salad_cat, group=user.group, client_id=str(uuid.uuid4()))
        db.session.add(meal)
        db.session.commit()
